...
function Billcounter(props) {
    ...
    let onDrag = (e, toucheEvent) => {
        ...
        if (toucheEvent) {
            ...
            if (e.target == inputWorked) objectsInteractivesOver.setObjectsWorked([...document.querySelectorAll(".billCounterNumbers")]);
            ...
            let interaccion = objectsInteractivesOver.verifyInteraction(e.target, clientX, clientY);
            if (interaccion) {
                onDragOver({
                    preventDefault: () => {},
                    target: interaccion
    ...
    let onDrop = (e) => {
        if (inputWorked == currentDivChoosen) {
            if (parseInt(inputWorked.value) > 0) {
                currentSuma = parseInt(inputWorked.value) - parseInt(e.target.innerText);
                if (currentSuma < 0) {
                    inputWorked.value = 0;
                    setCurrentSuma(0);
                } else {
    ...
    return <div className="billCounter_container">
        ...
        <div className="flexRowCenter" style={{ minHeight: "200px" }}>
            <input type="text" value={currentSuma} id="input_objectsInteractivesOver"
                // phones
                onTouchStart={onDragStart}
                onTouchEnd={e => onDragEnd(e, true)}
                onTouchMove={e => onDrag(e, true)}
